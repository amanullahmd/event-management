...
foreach ($file in $tsFiles) {
    ...
    if ($content -match "ticket" -and $content -match "import") {
        ...
        if ($content -match "@/modules/ticket-management") {
            $correctCount++
        }
    }
...
Write-Host ""
Write-Host "TypeScript import scan complete!" -ForegroundColor Green
...
if (Test-Path $typesIndexPath) {
    ...
    if ($content -match "modules/ticket-management/types/ticket") {
        Write-Host ""
        Write-Host "Main types index file correctly exports from ticket-management module" -ForegroundColor Green
    }
}

Write-Host ""
Write-Host "Frontend imports are already using the modular structure!" -ForegroundColor Green
